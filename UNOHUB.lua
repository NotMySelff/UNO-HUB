...
local function __unoPrintUnfavDiag()
    local env = (getgenv and getgenv()) or _G
    local d = type(env) == "table" and env.UNO_UNFAV_DIAG or nil
    if type(d) ~= "table" then
        print("[UNO UNFAV DIAG] no captured backend/UI error")
        return nil
    end
    print("[UNO UNFAV DIAG REPORT]")
    print("backend = " .. tostring(d.backend))
    print("ui = " .. tostring(d.ui))
    return d
end
...
do
    ...
    if type(env) == "table" then
        env.UNO_PRINT_UNFAV_DIAG = __unoPrintUnfavDiag
    end
end
...
print("[UNO UNFAV DIAG] BACKEND INSERT START")
local __unoUnfavBackendOk, __unoUnfavBackendErr = pcall(function()
-- AUTO UNFAVORITE CHICKENS (diagnostic exact clone)
--------------------------------------------------------------------
...
if __unoUnfavBackendOk then
    print("[UNO UNFAV DIAG] BACKEND INSERT OK")
else
    State._autoUnfavoriteFeature = nil
    local __msg = "[UNO UNFAV DIAG] BACKEND ERROR: " .. tostring(__unoUnfavBackendErr)
    warn(__msg)
    local __env = (getgenv and getgenv()) or _G
    if type(__env) == "table" then
        __env.UNO_UNFAV_DIAG = __env.UNO_UNFAV_DIAG or {}
        __env.UNO_UNFAV_DIAG.backend = __msg
    end
end
...
safeBuild("Auto Farm", function()
    ...
    print("[UNO UNFAV DIAG] UI INSERT START")
    local __unoUnfavUiOk, __unoUnfavUiErr = pcall(function()
    (function()
    ...
    if __unoUnfavUiOk then
        print("[UNO UNFAV DIAG] UI INSERT OK")
    else
        local __msg = "[UNO UNFAV DIAG] UI ERROR: " .. tostring(__unoUnfavUiErr)
        warn(__msg)
        local __env = (getgenv and getgenv()) or _G
        if type(__env) == "table" then
            __env.UNO_UNFAV_DIAG = __env.UNO_UNFAV_DIAG or {}
            __env.UNO_UNFAV_DIAG.ui = __msg
        end
    end
