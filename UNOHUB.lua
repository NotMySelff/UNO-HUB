...
local function __unoRunStage(name, fn)
    print("[UNO FINAL] START " .. tostring(name))

    if type(fn) ~= "function" then
        local msg = "[UNO TRACE] STAGE_NOT_CALLABLE name=" .. tostring(name) .. " type=" .. tostring(type(fn))
        warn(msg)
        error(msg, 0)
    end

    local function errorHandler(err)
        local base = "[UNO TRACE] STAGE=" .. tostring(name) .. " ERROR=" .. tostring(err)

        local tb = nil
        if type(debug) == "table" and type(debug.traceback) == "function" then
            local okTb, value = pcall(debug.traceback, tostring(err), 2)
            if okTb and type(value) == "string" then
                tb = value
            end
        end

        local full = tb and (base .. "\n" .. tb) or base
        warn(full)

        local env = (getgenv and getgenv()) or _G
        if type(env) == "table" then
            env.UNO_LAST_STAGE_ERROR = {
                stage = tostring(name),
                error = tostring(err),
                traceback = tb,
                report = full,
    ...
    print("[UNO FINAL] OK " .. tostring(name))
    return result
end

local function __unoPrintLastStageError()
    local env = (getgenv and getgenv()) or _G
    local data = type(env) == "table" and env.UNO_LAST_STAGE_ERROR or nil
    if type(data) ~= "table" then
        warn("[UNO TRACE] No captured stage error.")
        return nil
    end
    print(data.report or ("[UNO TRACE] " .. tostring(data.error)))
    return data
end
...
do
    local env = (getgenv and getgenv()) or _G
    if type(env) == "table" then
        env.UNO_PRINT_LAST_STAGE_ERROR = __unoPrintLastStageError
    end
end
