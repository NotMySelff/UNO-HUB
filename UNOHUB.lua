...
--[[
    ...
    Home Support V2
    Icon Toggle V1
    Automation Recovery Fix V1
    Delay + Toggle Fix V1
    Mobile Rounded UI Fix V1
    Auto Fuse + Target Material Filter Fix V1
]]
...
local State = {
    -- Keep new window-control instances on State instead of adding more top-level locals.
    -- Stage01 is already very close to Luau's 200-local-register limit.
    ...
    autoFarmRebirth = {
        ...
        -- User-adjustable delay. Default remains 30s.
        startDelay = 30,
        retryDelay = 30,
        -- Backend-only fixed cooldown after a successful rebirth.
        postRebirthCooldown = 5,
        countdown = 0, countdownLabel = "",
    ...
    toggles = {
        ...
        autoIncubatorClaim = false, autoSell = false, autoFavorite = false, autoUnfavorite = false, autoFuse = false, autoTargetFuse = false, autoCollectScrap = false,
...
local DEFAULT_CONFIG_NAME = "UNOHUB"
...
local function createConfigManager(deps)
    ...
    local deletefile = chooseFunction(fs, "delfile") or chooseFunction(fs, "deletefile")
    ...
    local function configPathForName(name)
        return CONFIG_FOLDER .. "/" .. sanitizeConfigName(name) .. ".json"
    end

    local function configPath()
        return configPathForName(config.configName)
    end
    ...
    function api.configExists(name)
        if not persistenceAvailable then return false end
        local ok, exists = pcall(isfile, configPathForName(name))
    ...
    function api.deleteConfig(name)
        ...
        local targetPath = configPathForName(name)
        local okExists, exists = pcall(isfile, targetPath)
        if not okExists then
            return false, "FILE CHECK FAILED"
        end
        ...
        local okDelete, deleteError = pcall(deletefile, targetPath)
...
State._targetFuseFeature = (function()
    ...
    local config = {
        ...
        targetId = nil,
        selectedRarities = {},
        -- Optional material chicken-name filter.
        -- Keys are chicken typeIds; empty means "all chicken names".
    ...
    local function availableMaterialTypes()
        local r = roster()
        local byType = {}

        for _, chicken in pairs(type(r) == "table" and r.chickens or {}) do
            if type(chicken) == "table" and chicken.id ~= nil and chicken.typeId ~= nil then
                local typeId = tostring(chicken.typeId)
                if byType[typeId] == nil then
                    byType[typeId] = {
                        id = typeId,
                        name = speciesName(chicken),
                        count = 0,
                    }
                end
    ...
    local function hasSelectedMaterialTypes()
        return next(config.selectedMaterialTypes) ~= nil
    end

    local function materialTypeAllowed(chicken)
        if not hasSelectedMaterialTypes() then
            return true
        end
        return chicken
            and chicken.typeId ~= nil
            and config.selectedMaterialTypes[tostring(chicken.typeId)] == true
    end
    ...
    local function materialCandidates(r, target)
        ...
        for _, chicken in pairs(type(r) == "table" and r.chickens or {}) do
            if type(chicken) == "table" and chicken.id ~= nil
                ...
                local rarity = resolvedRarity(chicken)
                if rarity
                    and config.selectedRarities[rarity] == true
                    and materialTypeAllowed(chicken) then
    ...
    function api.getAvailableMaterialTypes()
        return availableMaterialTypes()
    end

    function api.isMaterialTypeSelected(typeId)
        return config.selectedMaterialTypes[tostring(typeId or "")] == true
    end

    function api.setMaterialTypeSelected(typeId, enabled)
        typeId = tostring(typeId or "")
        if typeId == "" then return false end
        config.selectedMaterialTypes[typeId] = enabled == true or nil
        return true
    end

    function api.clearMaterialTypes()
        config.selectedMaterialTypes = {}
        return true
    end

    function api.getSelectedMaterialTypes()
        local out = {}
        for _, item in ipairs(availableMaterialTypes()) do
            if config.selectedMaterialTypes[tostring(item.id)] == true then
                table.insert(out, tostring(item.id))
            end
        end
        return out
    end
...
local function afrTick(token)
    local myGen = AFR.generation
        ...
        -- Recovery guard for stale coordinator ownership.
        -- Disabled features must never leave NORMAL_FARM permanently paused.
        if State.toggles.autoTower ~= true then
            AFR.coordinatorPauseReasons["COORDINATOR_OWNER:AUTO_TOWER"] = nil
        end
        if State.toggles.autoArena ~= true then
            AFR.coordinatorPauseReasons["COORDINATOR_OWNER:AUTO_ARENA"] = nil
        end
        if State.toggles.autoUfoAscension ~= true then
            AFR.coordinatorPauseReasons["COORDINATOR_OWNER:UFO_ASCENSION"] = nil
            AFR.coordinatorPauseReasons["UFO_EVENT_HOLD"] = nil
        end
        ...
        if ready then
            ...
            if requestRebirth(myGen) and myGen == AFR.generation and AFR.enabled then
                ...
                for i = AFR.postRebirthCooldown, 1, -1 do
                    if myGen ~= AFR.generation or not AFR.enabled then return end
                    AFR.countdown = i
                    task.wait(1)
                end
...
local function corner(parent, r)
    local radius = tonumber(r) or 8

    -- Mobile runs the whole UNO window through UIScale, which also shrinks
    -- pixel-based corner radii. Compensate on touch devices so cards, tabs,
    -- buttons and the main window retain the same rounded visual language
    -- as desktop instead of appearing square.
    if UserInputService.TouchEnabled then
        radius = math.max(radius, math.floor(radius * 1.75 + 0.5))
    end
...
local function stroke(parent, color)
    ...
    s.Thickness = UserInputService.TouchEnabled and 1.15 or 1
    pcall(function()
...
local function text(parent, str, size, color, font, xAlign)
    ...
    -- Readability pass: enlarge UI text without enlarging the whole window.
    size = size or 13
    if size > 1 then
        size = size + (size <= 13 and 2 or 1)
    end
    l.TextSize = size; l.TextColor3 = color or Theme.TextPrimary
    ...
    if ConfigManager then
        ConfigManager.registerSection("automation", function()
            return {
                ...
                autoFarmStartDelay = tonumber(AFR.startDelay) or 30,
                -- Legacy key kept so older UNO configs/tools remain compatible.
                autoFarmRebirthStartDelay = tonumber(AFR.startDelay) or 30,
                ...
                autoCollectScrap = State.toggles.autoCollectScrap == true,
        ...
        ConfigManager.registerSection("targetFuse", function()
            ...
            if not feature then return { enabled = false, targetId = "", rarities = "", materialTypes = "", locks = "" } end
            return {
                enabled = feature.isEnabled and feature.isEnabled() or false,
                targetId = tostring((feature.getTargetId and feature.getTargetId()) or ""),
                rarities = table.concat((feature.getSelectedRarities and feature.getSelectedRarities()) or {}, "\n"),
                materialTypes = table.concat((feature.getSelectedMaterialTypes and feature.getSelectedMaterialTypes()) or {}, "\n"),
            ...
            if feature.clearMaterialTypes then pcall(feature.clearMaterialTypes) end
            if type(data.materialTypes) == "string" and feature.setMaterialTypeSelected then
                for typeId in string.gmatch(data.materialTypes, "[^\n]+") do
                    pcall(feature.setMaterialTypeSelected, typeId, true)
                end
            end
        ...
        end, { defaults = { enabled = false, targetId = "", rarities = "", materialTypes = "", locks = "" } })
...
-- Personal profile wins when it exists; otherwise UNO HUB uses UNOHUB.json.
if ConfigManager then
    State._userConfigName = "UNOHUB_" .. tostring(LocalPlayer.Name)

    if ConfigManager.configExists
        and ConfigManager.configExists(State._userConfigName) then
        ConfigManager.setConfigName(State._userConfigName)
        State._usingUserConfig = true
    else
        ConfigManager.setConfigName("UNOHUB")
        State._usingUserConfig = false
    end
...
Sidebar.Size = UDim2.fromOffset(190, 620)
...
brand.Size = UDim2.new(1, 0, 0, 92)
brand.BackgroundTransparency = 1
brand.Parent = Sidebar

local brandUno = text(brand, "UnO", 24, Theme.TextPrimary, Enum.Font.GothamBold, Enum.TextXAlignment.Center)
brandUno.Position = UDim2.fromOffset(0, 12)
brandUno.Size = UDim2.new(1, 0, 0, 34)

local brandHub = text(brand, "HUB", 12, Theme.TextMuted, Enum.Font.GothamBold, Enum.TextXAlignment.Center)
brandHub.Position = UDim2.fromOffset(0, 46)
brandHub.Size = UDim2.new(1, 0, 0, 20)

local NavScroll = Instance.new("ScrollingFrame")
NavScroll.Position = UDim2.fromOffset(0, 92); NavScroll.Size = UDim2.new(1, 0, 1, -132)
...
Instance.new("UIListLayout", NavScroll).Padding = UDim.new(0, 9)
pad(NavScroll, 8, 12, 10, 12)
...
ContentRoot.Position = UDim2.fromOffset(190, 0); ContentRoot.Size = UDim2.new(1, -190, 1, 0)
...
State._resizeBtn = Instance.new("TextButton")
State._resizeBtn.Size = UDim2.fromOffset(30, 30); State._resizeBtn.Position = UDim2.new(1, -74, 0.5, -15)
State._resizeBtn.BackgroundColor3 = Theme.SurfaceElevated; State._resizeBtn.Text = "[ ]"; State._resizeBtn.Font = Enum.Font.GothamBold
State._resizeBtn.TextSize = 10; State._resizeBtn.TextColor3 = Theme.TextSecondary; State._resizeBtn.AutoButtonColor = false; State._resizeBtn.Parent = Topbar; corner(State._resizeBtn, 7)
...
Float.Name = "UNOToggle"
Float.Size = UDim2.fromOffset(64, 64)
Float.Position = UDim2.new(0, 18, 0.5, -32)

-- The uploaded PNG already contains the rounded purple frame/shadow.
-- Keep the Roblox button itself fully transparent so the toggle follows
-- the PNG instead of adding a second square/background around it.
Float.BackgroundTransparency = 1
Float.BorderSizePixel = 0
Float.Image = "rbxassetid://93424112073655"
...
-- Alternate uploaded asset ID if the primary image does not render:
-- Float.Image = "rbxassetid://71493340137849"
...
State._resizeBtn.MouseButton1Click:Connect(function()
    if State.closed then return end
    State.windowExpanded = not State.windowExpanded
    if State.windowExpanded then RESPONSIVE.baseW = 1180; RESPONSIVE.baseH = 700; Main.Size = UDim2.fromOffset(1180, 700); State._resizeBtn.Text = "[−]"
    else RESPONSIVE.baseW = 920; RESPONSIVE.baseH = 620; Main.Size = UDim2.fromOffset(920, 620); State._resizeBtn.Text = "[ ]" end
...
safeBuild("Home", function()
    ...
    local _, infoCard = card(sc, 1, "UNO HUB")

    local intro = text(
        infoCard,
        "Premium access, key management, and support",
        12,
        Theme.TextSecondary
    )
    intro.LayoutOrder = 1
    intro.Size = UDim2.new(1, 0, 0, 28)

    local serviceWrap = Instance.new("Frame")
    serviceWrap.LayoutOrder = 2
    serviceWrap.Size = UDim2.new(1, 0, 0, 116)
    serviceWrap.BackgroundTransparency = 1
    serviceWrap.Parent = infoCard

    local serviceList = Instance.new("UIListLayout")
    serviceList.Padding = UDim.new(0, 7)
    serviceList.Parent = serviceWrap

    for orderIndex, serviceName in ipairs({
        "Buy Premium Key",
        "Extend HWID",
        "Extend Expiration",
    }) do
        local serviceRow = Instance.new("Frame")
        serviceRow.LayoutOrder = orderIndex
        serviceRow.Size = UDim2.new(1, 0, 0, 30)
        serviceRow.BackgroundColor3 = Theme.SurfaceElevated
        serviceRow.BorderSizePixel = 0
        serviceRow.Parent = serviceWrap
        corner(serviceRow, 7)
        ...
        bullet.Position = UDim2.fromOffset(8, 0)
        bullet.Size = UDim2.fromOffset(20, 30)

        local serviceLabel = text(
            serviceRow,
            serviceName,
            12,
            Theme.TextPrimary,
        ...
        serviceLabel.Position = UDim2.fromOffset(34, 0)
        serviceLabel.Size = UDim2.new(1, -42, 1, 0)
    end

    local _, supportCard = card(sc, 2, "SUPPORT")

    local discordLabel = text(
        supportCard,
        "Discord: @1bad7",
        13,
        Theme.TextPrimary,
    ...
    local supportDesc = text(
        supportCard,
        "For purchases, HWID changes, expiration extensions, or account support.",
        11,
        Theme.TextMuted
    )
    supportDesc.LayoutOrder = 2
    supportDesc.Size = UDim2.new(1, 0, 0, 38)

    local actionRow = Instance.new("Frame")
    actionRow.LayoutOrder = 3
    actionRow.Size = UDim2.new(1, 0, 0, 40)
    actionRow.BackgroundTransparency = 1
    actionRow.Parent = supportCard

    local openDiscord = Instance.new("TextButton")
    openDiscord.Size = UDim2.new(0.5, -6, 1, 0)
    openDiscord.BackgroundColor3 = Theme.Primary
    openDiscord.BorderSizePixel = 0
    openDiscord.Text = "Open Discord"
    openDiscord.Font = Enum.Font.GothamBold
    openDiscord.TextSize = 11
    openDiscord.TextColor3 = Color3.fromRGB(18, 18, 18)
    openDiscord.AutoButtonColor = false
    openDiscord.Parent = actionRow
    corner(openDiscord, 7)

    local copyDiscord = Instance.new("TextButton")
    copyDiscord.Position = UDim2.new(0.5, 6, 0, 0)
    copyDiscord.Size = UDim2.new(0.5, -6, 1, 0)
    copyDiscord.BackgroundColor3 = Theme.SurfaceElevated
    copyDiscord.BorderSizePixel = 0
    copyDiscord.Text = "Copy Username"
    copyDiscord.Font = Enum.Font.GothamBold
    copyDiscord.TextSize = 11
    copyDiscord.TextColor3 = Theme.TextPrimary
    copyDiscord.AutoButtonColor = false
    copyDiscord.Parent = actionRow
    corner(copyDiscord, 7)
    stroke(copyDiscord)

    openDiscord.MouseButton1Click:Connect(function()
        local profileUrl = "https://discord.com/users/947311625214500956"
        local envNow = (getgenv and getgenv()) or _G

        local opener =
            (type(envNow) == "table" and (
                envNow.openurl
                or envNow.open_url
                or envNow.openbrowser
                or envNow.open_browser
            ))
            or (type(_G) == "table" and (
                _G.openurl
                or _G.open_url
                or _G.openbrowser
                or _G.open_browser
        ...
        if type(opener) == "function" then
            opened = pcall(opener, profileUrl)
        end

        if not opened then
            local clip =
                (type(envNow) == "table" and (envNow.setclipboard or envNow.toclipboard))
                or (type(_G) == "table" and (_G.setclipboard or _G.toclipboard))

            if type(clip) == "function" then
                pcall(clip, profileUrl)
                openDiscord.Text = "Link Copied"
            else
                openDiscord.Text = "Open Unsupported"
            end

            task.delay(1.5, function()
                if openDiscord and openDiscord.Parent then
                    openDiscord.Text = "Open Discord"
                end
            end)
        end
    ...
    copyDiscord.MouseButton1Click:Connect(function()
        ...
        if type(clip) == "function" then
            ...
            copyDiscord.Text = "Clipboard Unsupported"
        end

        task.delay(1.5, function()
            if copyDiscord and copyDiscord.Parent then
                copyDiscord.Text = "Copy Username"
            end
        end)
    end)
...
safeBuild("Auto Farm", function()
    ...
    State._delayStepper(farmCard, 2, "Start Delay", function()
        return AFR.startDelay
    end, function(value)
        -- User-adjustable Auto Farm delay. It does NOT affect the
        -- backend-only 5s post-rebirth cooldown.
        AFR.startDelay = value
    ...
    if AutoFuseFeature then
        ...
        autoFuseTrack, setAutoFuseVisual = makeSwitch(autoFuseRow, false, function(v)
            ...
            -- Auto Fuse V2 is user-triggered from this toggle.
            -- The feature boots in Dry Run for safety, so explicitly switch
            -- to real fusion only when the user turns Auto Fuse ON.
            if v == true and AutoFuseFeature.setDryRun then
                pcall(AutoFuseFeature.setDryRun, false)
            end
        ...
        if targetFeature then
            ...
            State._makeSummaryFilterSelector(targetCard, 4, "Material Chickens", function()
                local selected = targetFeature.getSelectedMaterialTypes and targetFeature.getSelectedMaterialTypes() or {}
                if #selected == 0 then return "All chicken names" end

                local names = {}
                local selectedMap = {}
                for _, typeId in ipairs(selected) do selectedMap[tostring(typeId)] = true end
                for _, item in ipairs(targetFeature.getAvailableMaterialTypes and targetFeature.getAvailableMaterialTypes() or {}) do
                    if selectedMap[tostring(item.id)] then
                        table.insert(names, tostring(item.name or item.id))
                    end
                end

                if #names == 0 then return tostring(#selected) .. " selected" end
                if #names <= 2 then return table.concat(names, ", ") end
                return tostring(#names) .. " selected"
            end, function(host, refreshSummary)
                local items = targetFeature.getAvailableMaterialTypes and targetFeature.getAvailableMaterialTypes() or {}
                if #items == 0 then
                    local empty = text(host, "No chickens detected in inventory", 11, Theme.TextMuted)
                    empty.LayoutOrder = 1
                ...
                for i, item in ipairs(items) do
                    local typeId = tostring(item.id)
                    local suffix = item.count and ("x" .. tostring(item.count)) or nil
                    makeFilterRow(
                        host,
                        i,
                        tostring(item.name or typeId),
                        suffix,
                        targetFeature.isMaterialTypeSelected and targetFeature.isMaterialTypeSelected(typeId) or false,
                        function(button)
                            local now = not (targetFeature.isMaterialTypeSelected and targetFeature.isMaterialTypeSelected(typeId))
                            if targetFeature.setMaterialTypeSelected then
                                targetFeature.setMaterialTypeSelected(typeId, now)
                            end
                            button.Text = now and "✓" or ""
                            refreshSummary()
                            markConfigDirty()
            ...
            lockRebuild, lockSummaryRefresh = State._makeSummaryFilterSelector(targetCard, 5, "Fusion Locks", function()
...
safeBuild("Configs", function()
    ...
    local profileStatus = row(cfgCard, 1, "Active Profile")

    local userSaveRow = Instance.new("Frame")
    userSaveRow.LayoutOrder = 2
    userSaveRow.Size = UDim2.new(1, 0, 0, 62)
    userSaveRow.BackgroundColor3 = Theme.SurfaceElevated
    userSaveRow.BorderSizePixel = 0
    userSaveRow.Parent = cfgCard
    corner(userSaveRow, 8)
    stroke(userSaveRow)

    local userSaveTitle = text(userSaveRow, "Save User Config", 13, Theme.TextPrimary, Enum.Font.GothamMedium)
    userSaveTitle.Position = UDim2.fromOffset(12, 5)
    userSaveTitle.Size = UDim2.new(1, -145, 0, 22)

    local userSaveDesc = text(
        userSaveRow,
        "Create this Roblox account's own config. Future changes save to it.",
        10,
        Theme.TextMuted
    )
    userSaveDesc.Position = UDim2.fromOffset(12, 27)
    userSaveDesc.Size = UDim2.new(1, -145, 0, 28)

    local userSaveButton = Instance.new("TextButton")
    userSaveButton.Size = UDim2.fromOffset(116, 34)
    userSaveButton.Position = UDim2.new(1, -128, 0.5, -17)
    userSaveButton.BackgroundColor3 = Theme.Primary
    userSaveButton.BorderSizePixel = 0
    userSaveButton.Text = "Save User"
    userSaveButton.Font = Enum.Font.GothamBold
    userSaveButton.TextSize = 11
    userSaveButton.TextColor3 = Color3.fromRGB(18, 18, 18)
    userSaveButton.AutoButtonColor = false
    userSaveButton.Parent = userSaveRow
    corner(userSaveButton, 7)

    local userDeleteRow = Instance.new("Frame")
    userDeleteRow.LayoutOrder = 3
    userDeleteRow.Size = UDim2.new(1, 0, 0, 62)
    userDeleteRow.BackgroundColor3 = Theme.SurfaceElevated
    userDeleteRow.BorderSizePixel = 0
    userDeleteRow.Parent = cfgCard
    corner(userDeleteRow, 8)
    stroke(userDeleteRow)

    local userDeleteTitle = text(userDeleteRow, "Delete User Config", 13, Theme.TextPrimary, Enum.Font.GothamMedium)
    userDeleteTitle.Position = UDim2.fromOffset(12, 5)
    userDeleteTitle.Size = UDim2.new(1, -145, 0, 22)

    local userDeleteDesc = text(
        userDeleteRow,
        "Remove only this account's config and return to shared UNOHUB.json.",
        10,
        Theme.TextMuted
    )
    userDeleteDesc.Position = UDim2.fromOffset(12, 27)
    userDeleteDesc.Size = UDim2.new(1, -145, 0, 28)

    local userDeleteButton = Instance.new("TextButton")
    userDeleteButton.Size = UDim2.fromOffset(116, 34)
    userDeleteButton.Position = UDim2.new(1, -128, 0.5, -17)
    userDeleteButton.BackgroundColor3 = Theme.Danger
    userDeleteButton.BorderSizePixel = 0
    userDeleteButton.Text = "Delete User"
    userDeleteButton.Font = Enum.Font.GothamBold
    userDeleteButton.TextSize = 11
    userDeleteButton.TextColor3 = Color3.new(1, 1, 1)
    userDeleteButton.AutoButtonColor = false
    userDeleteButton.Parent = userDeleteRow
    corner(userDeleteButton, 7)

    userSaveButton.MouseButton1Click:Connect(function()
        State._userConfigName = "UNOHUB_" .. tostring(LocalPlayer.Name)
        ConfigManager.setConfigName(State._userConfigName)
        local ok = ConfigManager.saveNow()
        if ok then
            State._usingUserConfig = true
            userSaveButton.Text = "Saved"
            task.delay(1.2, function()
                if userSaveButton and userSaveButton.Parent then
                    userSaveButton.Text = "Save User"
                end
            end)
        else
            userSaveButton.Text = "Save Failed"
            task.delay(1.5, function()
                if userSaveButton and userSaveButton.Parent then
                    userSaveButton.Text = "Save User"
                end
    ...
    userDeleteButton.MouseButton1Click:Connect(function()
        State._userConfigName = "UNOHUB_" .. tostring(LocalPlayer.Name)

        local ok = false
        if ConfigManager.deleteConfig then
            ok = select(1, ConfigManager.deleteConfig(State._userConfigName))
        end
        ...
        if ok then
            ...
            userDeleteButton.Text = "Deleted"
            task.delay(1.2, function()
                if userDeleteButton and userDeleteButton.Parent then
                    userDeleteButton.Text = "Delete User"
                end
            end)
        else
            userDeleteButton.Text = "Unavailable"
            task.delay(1.5, function()
                if userDeleteButton and userDeleteButton.Parent then
                    userDeleteButton.Text = "Delete User"
                end
    ...
    configName.PlaceholderText = "UNOHUB"
    ...
    local status = row(cfgCard, 10, "Status")
    local path = row(cfgCard, 11, "File")
    ...
    Views.Configs = {
        ...
        update = function()
            setText(
                profileStatus,
                State._usingUserConfig
                    and ("USER · " .. tostring(LocalPlayer.Name))
                    or "SHARED · UNOHUB"
            )
...
for i, p in ipairs(pages) do
    ...
    btn.Size = UDim2.new(1, 0, 0, 44); btn.BackgroundTransparency = 1; btn.Text = ""
    btn.AutoButtonColor = false; btn.LayoutOrder = i; btn.Parent = NavScroll; corner(btn, 8)
    local accent = Instance.new("Frame"); accent.Name = "Accent"; accent.Size = UDim2.fromOffset(4, 26)
    accent.Position = UDim2.fromOffset(0, 9); accent.BackgroundColor3 = Theme.Primary
    accent.BorderSizePixel = 0; accent.Visible = false; accent.Parent = btn; corner(accent, 2)
    local label = text(btn, (p.icon ~= "" and (p.icon .. "    ") or "") .. p.title, 13, Theme.TextSecondary, Enum.Font.GothamMedium)
    label.Name = "Label"; label.Position = UDim2.fromOffset(18, 0); label.Size = UDim2.new(1, -28, 1, 0)
...
local function create(deps)
    ...
    function api.enable()
        ...
        -- Self-heal stale coordinator pauses left by a previous event cycle.
        -- A genuinely active coordinator owner will re-assert its pause on
        -- the next sync tick, so clearing stale tokens here is safe.
        for reason in pairs(state.paused) do
            local reasonText = tostring(reason)
            if string.sub(reasonText, 1, 18) == "COORDINATOR_OWNER:" then
                state.paused[reason] = nil
            elseif reasonText == "UFO_EVENT_HOLD" then
                state.paused[reason] = nil
            end
        end

        if state.enabled then
            if next(state.paused) == nil then
                state.phase = "WAITING"
            end
            return true
        end
        ...
        state.phase = next(state.paused) == nil and "WAITING" or "PAUSED"
    ...
    function api.disable()
        ...
        for reason in pairs(state.paused) do
            local reasonText = tostring(reason)
            if string.sub(reasonText, 1, 18) == "COORDINATOR_OWNER:"
                or reasonText == "UFO_EVENT_HOLD" then
                state.paused[reason] = nil
            end
        end
